...
> **当前：M0 合约 + Mock，M1 Source 快照（git / local），M2 detect + plan + scope，以及 M3 多组件 start/stop。**  
> 这不是 Rocket.Chat 级验收。产品级证明是后续在操作者机器上、用户将该仓库标为 trusted 之后的可信运行，**不是 CI**。  
> CI 使用微型本地进程。不宣传对未信任目标的安全执行。
...
- 运行时 Discovery、真实 LLM
- M4–M6 其余适配器
- Effect DAG / Reconciler / 能力注册表 / Worker / Broker
- 在 CI 中克隆任何真实产品仓库

详见 [docs/M0.md](docs/M0.md)、[docs/M1.md](docs/M1.md)、[docs/M2.md](docs/M2.md) 与 [docs/M3.md](docs/M3.md)。
...
`npm test` 覆盖：结构 schema、语义校验、MockAgentRunner 合约、假数据走查、M1 SourceProvider（临时微型 git 夹具）、M2 detect/plan/scope，以及 M3 多组件 start/stop（微型本地进程，不克隆真实产品）。
